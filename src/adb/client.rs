...
use super::parser::{AdbDeviceInfo, parse_devices_output};
...
impl AdbClient {
    ...
    pub async fn shell_args(&self, serial: &str, args: &[&str]) -> Result<String> {
    ...
    pub async fn exec(&self, serial: &str, args: &[&str]) -> Result<Vec<u8>> {
        let mut command = Command::new(&self.adb_path);
    ...
    pub async fn is_device_online(&self, serial: &str) -> Result<bool> {
