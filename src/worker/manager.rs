...
impl WorkerManager {
    ...
    pub async fn sync_devices(&self, devices: &[AndroidDevice]) {
        for device in devices {
            ...
            let exists = {
                ...
                workers.contains_key(&device.serial)
            };

            if !exists {
                self.create_worker(&device.serial).await;
            }
        }
    }
    ...
    async fn create_worker(&self, serial: &str) {
        ...
        let (sender, receiver) = mpsc::channel::<Task>(self.queue_size);

        let busy = Arc::new(AtomicBool::new(false));

        let controller = DeviceController::new(self.adb.clone(), serial.to_string());

        let worker = DeviceWorker::new(controller, receiver, busy.clone());
        ...
        workers.insert(serial.to_string(), handle);
    ...
    pub async fn get(&self, serial: &str) -> Option<WorkerHandle> {
        ...
        workers.get(serial).map(|worker| WorkerHandle {
    ...
    pub async fn find_idle(&self, serials: &[String]) -> Option<WorkerHandle> {
        let workers = self.workers.read().await;

        for serial in serials {
            let Some(worker) = workers.get(serial) else {
                continue;
            };
